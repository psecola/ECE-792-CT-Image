...
def foram_import():
    ...
    directory = filepath_diag_box_pc("Please enter the CT scans directory:")
    ...
    if directory != '' and directory is not None:
        try:
            ...
            show_info_message_pc("No PKL file found. Resorting to importing scans via .tiff files and import function")
            ...
            show_info_message_pc(f"An error occurred while loading the PKL file: {e}. Resorting to importing scans via .tiff files and import function")
        ...
        show_info_message_pc("No directory provided")


    # Prompt user for CT scan metadata to compute chamber lengths
    user_answer = filepath_diag_box_pc("Do you have a file with image metadata to assist with computing measurements (Y/N):")

    if user_answer == "Y":
        fp = filepath_diag_box_pc("Please enter the CT metadata file path (see Github for file formatting):")

        if fp != '' and fp is not None:
            try:
                metadata = read_ct_header(fp)
                show_info_message_pc("Metadata import complete; import process complete")
                fp = None

            except FileNotFoundError:
                show_info_message_pc("No file found. Please try again or click OK to continue without inputting file")
                fp = filepath_diag_box_pc("Please enter the CT metadata file path (see Github for file formatting):")
                metadata = None

            except Exception as e:
                show_info_message_pc(f"An error occurred while loading the header file: {e}. Please try again or click OK to continue without loading a file")
                fp = filepath_diag_box_pc("Please enter the CT metadata file path (see Github for file formatting):")
                metadata = None
    else:
        metadata = None
        show_info_message_pc("Import Process complete")
