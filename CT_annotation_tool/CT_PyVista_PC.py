...
def save_slices(state):
    ...
    if not locked_state:
        ...
        slices_name = chamber_diag_box_pc()
        ...
        if slices_name is not None and slices_name != '':
            if slices_name.lower() not in [key.lower() for key in slices_dict.keys()]:
                ...
                try:
                    ...
                    show_info_message_pc("Slices saved")
                    ...
                    show_info_message_pc(f"Error {e}! Ensure all segmentations are completed before exiting. Slices not saved")
                ...
                show_info_message_pc("That name is taken. Please provide new name")

                # Unlock buttons
                locked_state = False

        else:
            show_info_message_pc("Invalid name provided. Process cancelled")
    ...
    if not locked_state:
        ...
        if len(masked_slices) != 0:
            try:
                ...
                direct = filepath_diag_box_pc("Input folder location annotations should be saved to")
                save_3d_annotations(im_dict=masked_slices, ct_array=condensed_stack, directory=direct, filename=fname, threshold=threshold)
                locked_state = False

                show_info_message_pc("Annotations saved")

            except FileNotFoundError:
                show_info_message_pc("Directory path not found. Save cancelled, please retry.")
                locked_state = False

            except Exception as e:
                show_info_message_pc(f"An issue occurred: {e}. Save cancelled, please retry.")
                locked_state = False

        else:
            show_info_message_pc("Please Save at least one set of annotation slices before saving collection")
            locked_state = False
...
def ellipsoid_fit(state):
    ...
    if not locked_state:
        ...
        file = filepath_diag_box_pc("Input .pkl file location of annotations that should be used")
        ...
        if file != '' and file is not None:
            try:
                ...
                show_info_message_pc("Chambers fitted")
                ...
                show_info_message_pc("No PKL file found. Please try again")

                # Unlock buttons
                locked_state = False

            except Exception as e:
                show_info_message_pc(f"An error {e} occurred while loading the PKL file. Please Ensure the annotations files is a PKL file")
...
def update_ellipsoid_dict(state):
    ...
    if not locked_state:
        ...
        user_input = filepath_diag_box_pc("Overwrite ellipsoid parameters (Y/N)?")
        ...
        if user_input == "Y":
            if 'ellip_params' in globals() and len('ellip_params') > 0:
                ...
                show_info_message_pc("Parameters updated")

                # Unlock buttons
                locked_state = False

            else:
                show_info_message_pc("Please annotate chambers first")
...
def export_chamber_params(state):
    ...
    if not locked_state:
        ...
        folder = filepath_diag_box_pc("Input folder location you would like to save chamber ellipsoid parameters CSV")
        ...
        if folder != '' and folder is not None:
            ...
            try:
                ...
                show_info_message_pc("Folder not found. Please try again.")

                # Unlock buttons
                locked_state = False

            except Exception as e:
                show_info_message_pc(f"An error occurred while saving the CSV file: {e}")

                # Unlock buttons
                locked_state = False

        else:
            show_info_message_pc("No file path entered. Please try again")
